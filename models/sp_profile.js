...
module.exports = conn => {
  ...
  function numberOfAssignedTickets() {
    return conn('tickets')
      .select('sp_assigned_id as id', conn.raw('count(distinct id)'))
      .groupBy('sp_assigned_id');
  }
  ...
  return {
    ...
    findById: async id => {
      ...
      let [profile] = await conn('sp_profiles').where('id', id).select();
    ...
    findByOpenId: async openid => {
      return await conn('sp_profiles').where('openid', openid).select();
    },

    find: async () => {
      let counts = await numberOfAssignedTickets();
      let index = R.indexBy(R.prop('id'), counts);
      let profiles = await conn('sp_profiles').select();
